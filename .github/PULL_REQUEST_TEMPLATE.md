## Summary

<!-- What does this PR change and why? -->

## Type of change

- [ ] Bug fix
- [ ] Accessibility improvement
- [ ] Performance improvement
- [ ] Typo / copy fix
- [ ] Other (describe below)

## Checklist

- [ ] Tested in Chrome or Firefox
- [ ] Tested on mobile (or DevTools responsive mode)
- [ ] No browser console errors
- [ ] Server starts cleanly (`npm install && cp .env.example .env && node server.js`)
- [ ] Does not modify personal content (bio, resume, contact info)
- [ ] Does not commit `data/config.json` changes
- [ ] Does not commit anything inside `data/uploads/`
- [ ] Does not commit `.env` (only `.env.example` changes are allowed)
- [ ] Does not touch `RECRUITER_CODE_PLACEHOLDER` or `ADMIN_PASS_PLACEHOLDER` in `gate.js`
- [ ] Does not commit `node_modules/`

## Screenshots (if visual change)

<!-- Before / After -->
